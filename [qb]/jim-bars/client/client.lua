...
RegisterNetEvent('jim-bars:washHands', function(data)
	if progressBar({label = Loc[Config.Lan].progressbar["progress_washing"], time = 5000, cancel = true, dict = "mp_arresting",	anim = "a_uncuff", flag = 8, icon = data.icon}) then
...
RegisterNetEvent('jim-bars:Crafting:MakeItem', function(data)
	...
	if progressBar({label = bartext..QBCore.Shared.Items[data.item].label, time = bartime, cancel = true, dict = animDictNow, anim = animNow, flag = 8, icon = data.icon}) then
				...
				if not QBCore.Shared.Items[k] then print("Item not found in server: '"..k.."'") else
					if (data.craftable[i]["job"] or data.craftable[i]["gang"]) and hasjob == false then else
						...
						Menu[#Menu + 1] = {
							...
							event = event, args = { item = k, craft = data.craftable[i], craftable = data.craftable, header = data.header, anim = data.craftable["anim"] }, -- ox_lib
						}
...
RegisterNetEvent('jim-bars:Crafting:MultiCraft', function(data)
	...
	for k in pairsByKeys(success) do
		Menu[#Menu + 1] = {
			...
			event = "jim-bars:Crafting:MakeItem", args = { item = data.item, craft = data.craft, craftable = data.craftable, header = data.header, amount = k },
			params = { event = "jim-bars:Crafting:MakeItem", args = { item = data.item, craft = data.craft, craftable = data.craftable, header = data.header, amount = k } }
		}
	end
	if Config.Menu == "ox" then	exports.ox_lib:registerContext({id = 'Crafting', title = data.header, position = 'top-right', options = Menu })	exports.ox_lib:showContext("Crafting")
	elseif Config.Menu == "qb" then	exports['qb-menu']:openMenu(Menu) end
