...
RegisterNetEvent('jim-mechanic:client:Repair:Check', function(skip)
	...
	if Config.UseMechJob == true then
		if exports['qb-mechanicjob']:GetVehicleStatus(QBCore.Functions.GetPlate(vehicle), "radiator") == nil then
		...
		for k, v in pairs(list) do
			print(v..": "..exports['qb-mechanicjob']:GetVehicleStatus(QBCore.Functions.GetPlate(vehicle), v))
		end
	end
		...
		if skip == -2 then else
			if not Config.FreeRepair then
				...
				if Config.UseMechJob then
					RadiatorRepair = Config.RepairRadiatorCost - math.floor(Config.RepairRadiatorCost * math.floor(exports['qb-mechanicjob']:GetVehicleStatus(QBCore.Functions.GetPlate(vehicle), "radiator")+0.5) / 100)
					...
					AxleRepair = Config.RepairAxleCost - math.floor(Config.RepairAxleCost * math.floor(exports['qb-mechanicjob']:GetVehicleStatus(QBCore.Functions.GetPlate(vehicle), "axle")+0.5) / 100)
					if AxleRepair ~= 0 then costAxle = Loc[Config.Lan]["repair"].cost..AxleRepair.." "..QBCore.Shared.Items[Config.RepairAxle].label end
					BrakesRepair = Config.RepairBrakesCost - math.floor(Config.RepairBrakesCost * math.floor(exports['qb-mechanicjob']:GetVehicleStatus(QBCore.Functions.GetPlate(vehicle), "brakes")+0.5) / 100)
					...
					ClutchRepair = Config.RepairClutchCost - math.floor(Config.RepairClutchCost * math.floor(exports['qb-mechanicjob']:GetVehicleStatus(QBCore.Functions.GetPlate(vehicle), "clutch")+0.5) / 100)
					...
					FuelRepair = Config.RepairFuelCost - math.floor(Config.RepairFuelCost * math.floor(exports['qb-mechanicjob']:GetVehicleStatus(QBCore.Functions.GetPlate(vehicle), "fuel")+0.5) / 100)
		...
		local RepairMenu = {
			...
			headerlock = false
			if Config.UseMechJob == true then
				if math.floor((exports['qb-mechanicjob']:GetVehicleStatus(QBCore.Functions.GetPlate(vehicle), "radiator"))+0.5) == 100 or skip == -2 then headerlock = true end	
				RepairMenu[#RepairMenu+1] =	{ isMenuHeader = headerlock, header = Loc[Config.Lan]["repair"].radiator, txt =  Loc[Config.Lan]["repair"].status..math.floor((exports['qb-mechanicjob']:GetVehicleStatus(QBCore.Functions.GetPlate(vehicle), "radiator"))+0.5).."%"..costRadiator, params = { event = "jim-mechanic:client:Repair:ItemCheck", args = { part = Loc[Config.Lan]["repair"].radiator, vehicle = vehicle, status = vehicleStatus, cost = RadiatorRepair } } }
				headerlock = false
				if math.floor((exports['qb-mechanicjob']:GetVehicleStatus(QBCore.Functions.GetPlate(vehicle), "axle"))+0.5) == 100 or skip == -2 then headerlock = true end	
				RepairMenu[#RepairMenu+1] =	{ isMenuHeader = headerlock, header = Loc[Config.Lan]["repair"].driveshaft, txt =  Loc[Config.Lan]["repair"].status..math.floor((exports['qb-mechanicjob']:GetVehicleStatus(QBCore.Functions.GetPlate(vehicle), "axle"))+0.5).."%"..costAxle, params = { event = "jim-mechanic:client:Repair:ItemCheck", args = { part = Loc[Config.Lan]["repair"].driveshaft, vehicle = vehicle, status = vehicleStatus, cost = AxleRepair } } }
				headerlock = false
				if math.floor((exports['qb-mechanicjob']:GetVehicleStatus(QBCore.Functions.GetPlate(vehicle), "brakes"))+0.5) == 100 or skip == -2 then headerlock = true end	
				RepairMenu[#RepairMenu+1] =	{ isMenuHeader = headerlock, header = Loc[Config.Lan]["repair"].brakes, txt =  Loc[Config.Lan]["repair"].status..math.floor((exports['qb-mechanicjob']:GetVehicleStatus(QBCore.Functions.GetPlate(vehicle), "brakes"))+0.5).."%"..costBrakes, params = { event = "jim-mechanic:client:Repair:ItemCheck", args = { part = Loc[Config.Lan]["repair"].brakes, vehicle = vehicle, status = vehicleStatus, cost = BrakesRepair } } }
				headerlock = false
				if math.floor((exports['qb-mechanicjob']:GetVehicleStatus(QBCore.Functions.GetPlate(vehicle), "clutch"))+0.5) == 100 or skip == -2 then headerlock = true end	
				...
				headerlock = false
				if math.floor((exports['qb-mechanicjob']:GetVehicleStatus(QBCore.Functions.GetPlate(vehicle), "fuel"))+0.5) == 100 or skip == -2 then headerlock = true end	
				RepairMenu[#RepairMenu+1] =	{ isMenuHeader = headerlock, header = Loc[Config.Lan]["repair"].tank, txt =  Loc[Config.Lan]["repair"].status..math.floor((exports['qb-mechanicjob']:GetVehicleStatus(QBCore.Functions.GetPlate(vehicle), "fuel"))+0.5).."%"..costFuel, params = { event = "jim-mechanic:client:Repair:ItemCheck", args = { part = Loc[Config.Lan]["repair"].tank, vehicle = vehicle, status = vehicleStatus, cost = FuelRepair } } }
			end
