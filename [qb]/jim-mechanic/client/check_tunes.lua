...
RegisterNetEvent('jim-mechanic:client:Menu', function()
	...
	local CheckMenu = {
		...
		if GetNumVehicleMods(vehicle,11) ~= 0 then
			...
			else modEngine = "<img src=nui://"..Config.img..QBCore.Shared.Items["engine"..(GetVehicleMod(vehicle, 11)+1)].image.." width=13px> "..QBCore.Shared.Items["engine"..(GetVehicleMod(vehicle, 11)+1)].label.." [LVL "..(GetVehicleMod(vehicle, 11)+1).."]" modEngineHead = false end
		...
		if GetNumVehicleMods(vehicle, 12) ~= 0 then
			...
			else modBrakes = "<img src=nui://"..Config.img..QBCore.Shared.Items["brakes"..(GetVehicleMod(vehicle, 12)+1)].image.." width=13px> "..QBCore.Shared.Items["brakes"..(GetVehicleMod(vehicle, 12)+1)].label.." [LVL "..(GetVehicleMod(vehicle, 12)+1).."]" modBrakesHead = false end
		...
		if GetNumVehicleMods(vehicle,15) ~= 0 then 
			...
			else modSuspension = "<img src=nui://"..Config.img..QBCore.Shared.Items["suspension"..(GetVehicleMod(vehicle, 15)+1)].image.." width=13px> "..QBCore.Shared.Items["suspension"..(GetVehicleMod(vehicle, 15)+1)].label.." [LVL "..(GetVehicleMod(vehicle, 15)+1).."]" modSuspensionHead = false end
		...
		if GetNumVehicleMods(vehicle,13) ~= 0 then 
			...
			else modTransmission = "<img src=nui://"..Config.img..QBCore.Shared.Items["transmission"..(GetVehicleMod(vehicle, 13)+1)].image.." width=13px> "..QBCore.Shared.Items["transmission"..(GetVehicleMod(vehicle, 13)+1)].label modTransmissionHead = false end
		else modTransmission = Loc[Config.Lan]["check"].unavail modTransmissionHead = true end
		--Armor
		if GetNumVehicleMods(vehicle, 16) ~= 0 then 
			if GetVehicleMod(vehicle, 16)+1 == GetNumVehicleMods(vehicle, 16) then modArmor = "<img src=nui://"..Config.img..QBCore.Shared.Items["car_armor"].image.." width=13px> "..Loc[Config.Lan]["check"].reinforced modArmorHead = false
		...
		elseif IsToggleModOn(vehicle, 18) then modTurbo = "<img src=nui://"..Config.img..QBCore.Shared.Items["turbo"].image.." width=13px> "..QBCore.Shared.Items["turbo"].label modTurboHead = false
		...
		elseif IsToggleModOn(vehicle, 22) then modXenon = "<img src=nui://"..Config.img..QBCore.Shared.Items["headlights"].image.." width=13px> "..Loc[Config.Lan]["check"].xenoninst modXenonHead = false
		...
		elseif GetDriftTyresEnabled(vehicle) == 1 then modDrift = "<img src=nui://"..Config.img..QBCore.Shared.Items["drifttires"].image.." width=13px> "..Loc[Config.Lan]["check"].tireinst modDriftHead = false
		...
		elseif GetVehicleTyresCanBurst(vehicle) == false and GetNumVehicleMods(vehicle,11) ~= 0 then modBproof = "<img src=nui://"..Config.img..QBCore.Shared.Items["bprooftires"].image.." width=13px> "..Loc[Config.Lan]["check"].tireinst modBproofHead = false
		elseif GetNumVehicleMods(vehicle,11) == 0 then modBproof = Loc[Config.Lan]["check"].unavail modBproofHead = true  end
		
		if IsVehicleOwned(trim(GetVehicleNumberPlateText(vehicle))) then local VehicleNitrous = nil while VehicleNitrous == nil do QBCore.Functions.TriggerCallback('jim-mechanic:GetNosLoaded', function(vehs) VehicleNitrous = vehs end, trim(GetVehicleNumberPlateText(vehicle))) Wait(100) end
			if VehicleNitrous[trim(GetVehicleNumberPlateText(vehicle))] == nil then else
				CheckMenu[#CheckMenu + 1] = { header = Loc[Config.Lan]["check"].label58, txt = "<img src=nui://"..Config.img..QBCore.Shared.Items["nos"].image.." width=13px> LVL: "..VehicleNitrous[trim(GetVehicleNumberPlateText(vehicle))].level.."%", params = { event = "jim-mechanic:client:Menu:Remove", args = { vehicle = vehicle, mod = "nos" } } }
			end
		end

		CheckMenu[#CheckMenu + 1] = { isMenuHeader = modEngineHead, header = Loc[Config.Lan]["check"].label1, txt = modEngine, params = { event = "jim-mechanic:client:Menu:Remove", args = { vehicle = vehicle, mod = "engine" } } }
		CheckMenu[#CheckMenu + 1] = { isMenuHeader = modBrakesHead, header = Loc[Config.Lan]["check"].label2, txt = modBrakes, params = { event = "jim-mechanic:client:Menu:Remove", args = { vehicle = vehicle, mod = "brakes" } } }	
		CheckMenu[#CheckMenu + 1] = { isMenuHeader = modSuspensionHead, header = Loc[Config.Lan]["check"].label3, txt = modSuspension, params = { event = "jim-mechanic:client:Menu:Remove", args = { vehicle = vehicle, mod = "suspension" } } }
		CheckMenu[#CheckMenu + 1] = { isMenuHeader = modTransmissionHead, header = Loc[Config.Lan]["check"].label4, txt = modTransmission, params = { event = "jim-mechanic:client:Menu:Remove", args = { vehicle = vehicle, mod = "transmission" } } }
		CheckMenu[#CheckMenu + 1] = { isMenuHeader = modArmorHead, header = Loc[Config.Lan]["check"].label5, txt = modArmor, params = { event = "jim-mechanic:client:Menu:Remove", args = { vehicle = vehicle, mod = "armour" } } }
		CheckMenu[#CheckMenu + 1] = { isMenuHeader = modTurboHead, header = Loc[Config.Lan]["check"].label6, txt = modTurbo, params = { event = "jim-mechanic:client:Menu:Remove", args = { vehicle = vehicle, mod = "turbo" } } }
		CheckMenu[#CheckMenu + 1] = { isMenuHeader = modXenonHead, header = Loc[Config.Lan]["check"].label7, txt = modXenon..xenonColor, params = { event = "jim-mechanic:client:Menu:Remove", args = { vehicle = vehicle, mod = "xenon" } } }
		CheckMenu[#CheckMenu + 1] = { isMenuHeader = modDriftHead, header = Loc[Config.Lan]["check"].label8, txt = modDrift, params = { event = "jim-mechanic:client:Menu:Remove", args = { vehicle = vehicle, mod = "drift" } } }
		CheckMenu[#CheckMenu + 1] = { isMenuHeader = modBproofHead, header = Loc[Config.Lan]["check"].label9, txt = modBproof, params = { event = "jim-mechanic:client:Menu:Remove", args = { vehicle = vehicle, mod = "bproof" } } }
		

		CheckMenu[#CheckMenu + 1] = { header = "", txt = "🧰 - "..Loc[Config.Lan]["check"].label10, params = { event = "jim-mechanic:client:Menu:List" } }
...
RegisterNetEvent('jim-mechanic:client:Menu:Remove', function(data)
	local CheckMenu = {
		...
		if data.mod == "brakes" then 
			CheckMenu[#CheckMenu+1] = { isMenuHeader = true, header = "<img src=nui://"..Config.img..QBCore.Shared.Items["brakes"..(GetVehicleMod(data.vehicle, 12)+1)].image.." width=50px>"..Loc[Config.Lan]["check"].label49 }
			CheckMenu[#CheckMenu+1] = { header = "", txt = Loc[Config.Lan]["check"].label47, params = { event = "jim-mechanic:client:giveBrakes" } } end
		if data.mod == "engine" then 
			CheckMenu[#CheckMenu+1] = { isMenuHeader = true, header = "<img src=nui://"..Config.img..QBCore.Shared.Items["engine"..(GetVehicleMod(data.vehicle, 11)+1)].image.." width=50px>"..Loc[Config.Lan]["check"].label50 }
			CheckMenu[#CheckMenu+1] = { header = "", txt = Loc[Config.Lan]["check"].label47, params = { event = "jim-mechanic:client:giveEngine" } } end
		if data.mod == "suspension" then 
			CheckMenu[#CheckMenu+1] = { isMenuHeader = true, header = "<img src=nui://"..Config.img..QBCore.Shared.Items["suspension"..(GetVehicleMod(data.vehicle, 15)+1)].image.." width=50px>"..Loc[Config.Lan]["check"].label51 }
			CheckMenu[#CheckMenu+1] = { header = "", txt = Loc[Config.Lan]["check"].label47, params = { event = "jim-mechanic:client:giveSuspension" } } end
		if data.mod == "transmission" then 
			CheckMenu[#CheckMenu+1] = { isMenuHeader = true, header = "<img src=nui://"..Config.img..QBCore.Shared.Items["transmission"..(GetVehicleMod(data.vehicle, 13)+1)].image.." width=50px>"..Loc[Config.Lan]["check"].label52 }
			CheckMenu[#CheckMenu+1] = { header = "", txt = Loc[Config.Lan]["check"].label47, params = { event = "jim-mechanic:client:giveTransmission" } } end
		if data.mod == "armour" then 
			CheckMenu[#CheckMenu+1] = { isMenuHeader = true, header = "<img src=nui://"..Config.img..QBCore.Shared.Items["car_armor"].image.." width=50px>"..Loc[Config.Lan]["check"].label53 }
			CheckMenu[#CheckMenu+1] = { header = "", txt = Loc[Config.Lan]["check"].label47, params = { event = "jim-mechanic:client:giveArmor" } } end
		if data.mod == "turbo" then 
			CheckMenu[#CheckMenu+1] = { isMenuHeader = true, header = "<img src=nui://"..Config.img..QBCore.Shared.Items["turbo"].image.." width=50px>"..Loc[Config.Lan]["check"].label54 }
			CheckMenu[#CheckMenu+1] = { header = "", txt = Loc[Config.Lan]["check"].label47, params = { event = "jim-mechanic:client:giveTurbo" } } end
		if data.mod == "xenon" then 
			CheckMenu[#CheckMenu+1] = { isMenuHeader = true, header = "<img src=nui://"..Config.img..QBCore.Shared.Items["headlights"].image.." width=50px>"..Loc[Config.Lan]["check"].label55 }
			CheckMenu[#CheckMenu+1] = { header = "", txt = Loc[Config.Lan]["check"].label47, params = { event = "jim-mechanic:client:giveXenon" } } end
		if data.mod == "drift" then 
			CheckMenu[#CheckMenu+1] = { isMenuHeader = true, header = "<img src=nui://"..Config.img..QBCore.Shared.Items["drifttires"].image.." width=50px>"..Loc[Config.Lan]["check"].label56 }
			CheckMenu[#CheckMenu+1] = { header = "", txt = Loc[Config.Lan]["check"].label47, params = { event = "jim-mechanic:client:giveDrift" } } end
		if data.mod == "bproof" then 
			CheckMenu[#CheckMenu+1] = { isMenuHeader = true, header = "<img src=nui://"..Config.img..QBCore.Shared.Items["bprooftires"].image.." width=50px>"..Loc[Config.Lan]["check"].label56 }
			CheckMenu[#CheckMenu+1] = { header = "", txt = Loc[Config.Lan]["check"].label47, params = { event = "jim-mechanic:client:giveBulletProof" } } end
		if data.mod == "nos" then 
			CheckMenu[#CheckMenu+1] = { isMenuHeader = true, header = "<img src=nui://"..Config.img..QBCore.Shared.Items["noscan"].image.." width=50px>"..Loc[Config.Lan]["check"].label57 }
			CheckMenu[#CheckMenu+1] = { header = "", txt = Loc[Config.Lan]["check"].label47, params = { event = "jim-mechanic:client:giveNOS" } } end
