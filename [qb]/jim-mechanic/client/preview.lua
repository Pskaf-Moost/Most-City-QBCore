...
local function printDifferences(vehicle, properties, newproperties)
	...
	if properties["modBackWheels"] ~= newproperties["modBackWheels"] then
		...
		originalWheel = GetVehicleWheelType(vehicle)
		SetVehicleWheelType(vehicle, newproperties["wheels"])
		vehlist[#vehlist+1] = "Wheels -[ '"..GetLabelText(GetModTextLabel(vehicle, 24, (newproperties["modBackWheels"]))).."' - ("..name..") ]"
		SetVehicleWheelType(vehicle, originalWheel)
	end
...
RegisterNetEvent('jim-mechanic:client:Preview:Livery', function()
	local playerPed = PlayerPedId()
	local validMods = {}
	local vehicle = nil
	if IsPedInAnyVehicle(playerPed, false) then	vehicle = GetVehiclePedIsIn(playerPed, false) pushVehicle(vehicle) 
	...
	if DoesEntityExist(vehicle) then
		...
		if oldlivery == true then
				...
				LiveryMenu[#LiveryMenu + 1] = { header = "", txt = Loc[Config.Lan]["common"].ret, params = { event = "jim-mechanic:client:Preview:Menu" }, }
				LiveryMenu[#LiveryMenu + 1] = {  header = "0. "..Loc[Config.Lan]["common"].stock, txt = stockinstall, params = { event = "jim-mechanic:client:Preview:Apply", args = { id = tostring(0), old = true } } }
			for k,v in pairs(validMods) do
				LiveryMenu[#LiveryMenu + 1] = { header = k..". "..v.name, txt = v.install, params = { event = 'jim-mechanic:client:Preview:Apply', args = { id = tostring(v.id), old = true } } }
			end
		elseif oldlivery ~= true then
				...
				LiveryMenu[#LiveryMenu + 1] = { header = "", txt = Loc[Config.Lan]["common"].ret, params = { event = "jim-mechanic:client:Preview:Menu" }, }
				LiveryMenu[#LiveryMenu + 1] = {  header = "0. "..Loc[Config.Lan]["common"].stock, txt = stockinstall, params = { event = "jim-mechanic:client:Preview:Apply", args = { id = tostring(-1) } } }
			for k,v in pairs(validMods) do
				LiveryMenu[#LiveryMenu + 1] = { header = k..". "..v.name, txt = v.install, params = { event = 'jim-mechanic:client:Preview:Apply', args = { id = tostring(v.id) } } }
			end
		end
...
RegisterNetEvent('jim-mechanic:client:Preview:Rims:Choose', function(data)
	...
	for i = 1, (GetNumVehicleMods(vehicle, 23) +1) do
		...
		for k, v in pairs(validMods) do
			if not validMods[k][1] then
				RimsMenu[#RimsMenu + 1] = { header = v.name, txt = v.install, params = { event = 'jim-mechanic:client:Preview:Rims:Apply', args = { mod = v.id, wheeltype = data.wheeltype, wheelname = GetLabelText(GetModTextLabel(vehicle, 23, GetVehicleMod(vehicle, 23))) } } }
			else
				if k == "NULL" then setheader = Loc[Config.Lan]["rims"].labelcustom else setheader = k end
				RimsMenu[#RimsMenu + 1] = { header = setheader, txt = Loc[Config.Lan]["common"].amountoption..#validMods[k], params = { event = 'jim-mechanic:client:Preview:Rims:SubMenu', args = { mod = v.id, wheeltype = data.wheeltype, wheeltable = validMods[k], bike = data.bike } } }
			end
		end
