...
CreateThread(function()
	for k, v in pairs(Config.PoliceLocations) do
		...
		exports['qb-target']:AddBoxZone("bench"..k, v.coords, 1.2, 4.2, { name="bench"..k, heading = v.heading, debugPoly=Config.Debug, minZ = v.coords.z-1, maxZ = v.coords.z+1.4, }, 
			{ options = { { event = "jim-mechanic:client:Police:Menu", icon = "fas fa-cogs", label = "Use Repair Station" , job = Config.QuickJobs, }, }, distance = 5.0 })
	end
end)
...
RegisterNetEvent('jim-mechanic:client:Police:Menu', function()
	...
	if IsPedInAnyVehicle(playerPed, false) then
		local PoliceMenu = {}
			PoliceMenu[#PoliceMenu+1] = { header = Loc[Config.Lan]["common"].close, params = { event = "jim-mechanic:client:Menu:Close" } }
			PoliceMenu[#PoliceMenu+1] = { header = Loc[Config.Lan]["police"].repair, txt = "", params = { event = "jim-mechanic:client:Police:Repair" }, }
			PoliceMenu[#PoliceMenu+1] = { header = Loc[Config.Lan]["police"].extras, txt = "", params = { event = "jim-mechanic:client:Police:Extra" }, }
			PoliceMenu[#PoliceMenu+1] = { header = Loc[Config.Lan]["police"].plates, txt = "", params = { event = "jim-mechanic:client:Police:Plates" }, }
			if GetNumVehicleMods(vehicle, 48) > 0 or GetVehicleLiveryCount(vehicle) > -1 then
				PoliceMenu[#PoliceMenu+1] = { header = Loc[Config.Lan]["police"].livery, txt = "", params = { event = "jim-mechanic:client:Police:Livery" }, } end
			if GetNumVehicleMods(vehicle, 0) ~= 0 then 
				PoliceMenu[#PoliceMenu+1] = { header = Loc[Config.Lan]["police"].spoiler, txt = "", params = { event = "jim-mechanic:client:Police:Spoiler" }, } end
			PoliceMenu[#PoliceMenu+1] = { header = Loc[Config.Lan]["paint"].menuheader, txt = "", params = { event = "jim-mechanic:client:Police:Paint" }, }
...
RegisterNetEvent('jim-mechanic:client:Police:Extra', function()
	...
	ExtraMenu[#ExtraMenu+1] = { header = "", txt = Loc[Config.Lan]["common"].ret, params = { event = "jim-mechanic:client:Police:Menu" }, }
	for i = 0, 20 do
		...
		ExtraMenu[#ExtraMenu+1] = { header = setheader, txt = "", params = { event = "jim-mechanic:client:Police:Extra:Apply", args = { id = i }, }, } end
	end
...
RegisterNetEvent('jim-mechanic:client:Police:Repair', function()
	...
	TriggerEvent("QBCore:Notify", "Repairing Engine...")
	SetVehicleEngineHealth(vehicle, 1000.0)
	SetVehicleBodyHealth(vehicle, 1000.0)
	Wait(2000)
	TriggerEvent("QBCore:Notify", "Repairing Body...")
	...
	Wait(2000)
	TriggerEvent("QBCore:Notify", "Cleaning Car...")
	local cleaning = true								
	while cleaning do
...
RegisterNetEvent('jim-mechanic:client:Police:Livery', function()
	...
	if DoesEntityExist(vehicle) then
		...
		if oldlivery == true then
				...
				LiveryMenu[#LiveryMenu + 1] = { header = "", txt = Loc[Config.Lan]["common"].ret, params = { event = "jim-mechanic:client:Police:Menu" }, }
				LiveryMenu[#LiveryMenu + 1] = {  header = "0. "..Loc[Config.Lan]["common"].stock, txt = stockinstall, params = { event = "jim-mechanic:client:Police:Apply", args = { id = tostring(0), old = true } } }
			for k,v in pairs(validMods) do
				LiveryMenu[#LiveryMenu + 1] = { header = k..". "..v.name, txt = v.install, params = { event = 'jim-mechanic:client:Police:Apply', args = { id = tostring(v.id), old = true } } }
			end
		elseif oldlivery ~= true then
				...
				LiveryMenu[#LiveryMenu + 1] = { header = "", txt = Loc[Config.Lan]["common"].ret, params = { event = "jim-mechanic:client:Police:Menu" }, }
				LiveryMenu[#LiveryMenu + 1] = {  header = "0. "..Loc[Config.Lan]["common"].stock, txt = stockinstall, params = { event = "jim-mechanic:client:Police:Apply", args = { id = tostring(-1) } } }
			for k,v in pairs(validMods) do
				LiveryMenu[#LiveryMenu + 1] = { header = k..". "..v.name, txt = v.install, params = { event = 'jim-mechanic:client:Police:Apply', args = { id = tostring(v.id) } } }
			end
		end
...
RegisterNetEvent('jim-mechanic:client:Police:Plates', function()
	...
	if IsPedInAnyVehicle(playerPed, false) then	vehicle = GetVehiclePedIsIn(playerPed, false) pushVehicle(vehicle) 
		if DoesEntityExist(vehicle) then
			...
			{ header = "", txt = Loc[Config.Lan]["common"].ret, params = { event = "jim-mechanic:client:Police:Menu" } } }
			for k, v in pairs(Loc[Config.Lan].vehiclePlateOptions) do
				...
				PlateMenu[#PlateMenu + 1] = { header = k..". "..v.name, txt = installed, params = { event = 'jim-mechanic:client:Police:Plates:Apply', args = v.id  } }
			end
...
RegisterNetEvent('jim-mechanic:client:Police:Spoiler', function()
	...
	if DoesEntityExist(vehicle) then
		...
		local spoilerMenu = {
				...
				{ header = "", txt = Loc[Config.Lan]["common"].ret, params = { event = "jim-mechanic:client:Police:Menu" } },
				{ header = "0. "..Loc[Config.Lan]["common"].stock, txt = stockinstall,	params = { event = "jim-mechanic:client:Police:Spoilers:Apply", args = -1 } } }
			for k,v in pairs(validMods) do
				spoilerMenu[#spoilerMenu + 1] = { header = k..". "..v.name, txt = v.install, params = { event = 'jim-mechanic:client:Police:Spoilers:Apply', args = tostring(v.id) } }
			end
...
RegisterNetEvent('jim-mechanic:client:Police:Paints:Choose', function(data)
	...
	if DoesEntityExist(vehicle) then
		exports['qb-menu']:openMenu({
			...
			{ header = "", txt = Loc[Config.Lan]["common"].ret, params = { event = "jim-mechanic:client:Police:Paint" } },
...
RegisterNetEvent('jim-mechanic:client:Police:Paints:Choose:Colour', function(data)
	...
	local PaintMenu = {	
		...
		{ header = "", txt = Loc[Config.Lan]["common"].ret, params = { event = "jim-mechanic:client:Police:Paints:Choose", args = data.paint } } }
	local installed = nil
	if data.finish == Loc[Config.Lan]["paint"].classic then
		for k, v in pairs(Loc[Config.Lan].vehicleResprayOptionsClassic) do
			...
			PaintMenu[#PaintMenu + 1] = { header = k..". "..v.name, txt = installed, params = { event = 'jim-mechanic:client:Police:Paints:Apply', args = { paint = data.paint, id = v.id, name = v.name, finish = data.finish } } } end
...
AddEventHandler('onResourceStop', function(r) 
	if r == GetCurrentResourceName() then 
		for k, v in pairs(Config.RegisterLocations) do exports['qb-target']:RemoveZone("bench"..k) end
